import static org.junit.Assert.*;
import org.junit.Test;

import java.util.HashSet;

public class HashMapTest {

    @Test
    public void testConstructor() {
        // assert constructors are initialized, seem to work correctly, and
        // don't cause errors
        HashMap<String, String> dictionary = new HashMap<String, String>();
        assertEquals(0, dictionary.size());
        assertEquals(16, dictionary.capacity());

        dictionary = new HashMap<String, String>(10);
        assertEquals(0, dictionary.size());
        assertEquals(10, dictionary.capacity());

        // simply test that the constructor exists, resizeTest will do the rest
        dictionary = new HashMap<String, String>(10, 1);
        assertEquals(0, dictionary.size());
        assertEquals(10, dictionary.capacity());
    }

    @Test
    public void testClear() {
        HashMap<String, String> h = new HashMap<String, String>();
        h.put("ryan", "pranav");
        assertTrue(h.containsKey("ryan"));
        assertEquals("pranav", h.get("ryan"));
        assertEquals(1, h.size());
        h.put("matt", "pancakes");
        assertTrue(h.containsKey("matt"));
        assertEquals("pancakes", h.get("matt"));
        assertEquals(2, h.size());
        h.clear();
        assertFalse(h.containsKey("ryan"));
        assertFalse(h.containsKey("matt"));
        assertEquals(0, h.size());
    }

    @Test
    public void testPut() {
        HashMap<String, String> h = new HashMap<String, String>();
        h.put("lauren", "wallace");
        assertTrue(h.containsKey("lauren"));
        assertEquals("wallace", h.get("lauren"));
        assertFalse(h.containsKey("ervin"));
        assertFalse(h.containsKey("lucas"));
        assertEquals(1, h.size());
        h.put("eli", "josh");
        assertTrue(h.containsKey("eli"));
        assertFalse(h.containsKey("josh"));
        assertEquals(2, h.size());
    }

    @Test
    public void testGet() {
        HashMap<String, String> h = new HashMap<String, String>();
        h.put("ryan", "aditya");
        assertEquals("aditya", h.get("ryan"));
    }

    @Test
    public void testRemove() {
        HashMap<String, String> h = new HashMap<String, String>();
        h.put("alex", "claire");
        assertTrue(h.containsKey("alex"));
        assertEquals("claire", h.get("alex"));
        assertEquals(1, h.size());
        h.remove("alex");
        assertFalse(h.containsKey("alex"));
        assertEquals(0, h.size());
    }

    @Test
    public void testResize() {
        HashMap<String, String> h = new HashMap<String, String>(2);
        assertEquals(2, h.capacity());
        h.put("charles", "won");
        h.put("christine", "matt");
        assertEquals(4, h.capacity());

        h = new HashMap<String, String>(10, 1);
        for (int i = 1; i <= 10; i += 1) {
            h.put(Integer.toString(i), Integer.toString(i));
        }
        assertEquals(10, h.size());
        assertEquals(10, h.capacity());
        h.put("matt", "matt");
        assertEquals(11, h.size());
        assertEquals(20, h.capacity());
        h.remove("matt");
        assertEquals(10, h.size());
        assertEquals(20, h.capacity());
    }

    @Test
    public void basicFunctionalityTest() {
        HashMap<String, String> dictionary = new HashMap<String, String>();
        assertEquals(0, dictionary.size());
        assertEquals(16, dictionary.capacity());

        // can put objects in dictionary and get them
        dictionary.put("won", "charles");
        assertTrue(dictionary.containsKey("won"));
        assertEquals("charles", dictionary.get("won"));
        assertEquals(1, dictionary.size());

        // putting with existing key replaces key
        dictionary.put("won", "brandon");
        assertEquals(1, dictionary.size());
        assertEquals("brandon", dictionary.get("won"));
        assertEquals("brandon", dictionary.remove("won"));
        assertEquals(null, dictionary.get("won"));
        assertEquals(0, dictionary.size());

        // placing key in multiple times does not affect behavior
        HashMap<String, Integer> studentIDs = new HashMap<String, Integer>();
        studentIDs.put("christine", 12345);
        assertEquals(1, studentIDs.size());
        assertEquals(12345, studentIDs.get("christine").intValue());
        studentIDs.put("julianna", 345);
        assertEquals(2, studentIDs.size());
        assertEquals(12345, studentIDs.get("christine").intValue());
        assertEquals(345, studentIDs.get("julianna").intValue());
        studentIDs.put("julianna", 345);
        assertEquals(2, studentIDs.size());
        assertEquals(12345, studentIDs.get("christine").intValue());
        assertEquals(345, studentIDs.get("julianna").intValue());
        studentIDs.put("julianna", 345);
        assertEquals(2, studentIDs.size());
        assertEquals(12345, studentIDs.get("christine").intValue());
        assertEquals(345, studentIDs.get("julianna").intValue());
        assertTrue(studentIDs.containsKey("christine"));
        assertTrue(studentIDs.containsKey("julianna"));

        // ensure that containsKey does not always return true
        assertFalse(studentIDs.containsKey("joe"));
        assertFalse(studentIDs.containsKey("gigi"));
        studentIDs.put("gigi", 612);
        studentIDs.put("joe", 216);
        assertTrue(studentIDs.containsKey("gigi"));
        assertTrue(studentIDs.containsKey("joe"));

        // confirm hash map can handle values being the same
        assertEquals(345, studentIDs.get("julianna").intValue());
        studentIDs.put("jackson", 345);
        assertEquals(345, studentIDs.get("jackson").intValue());
        assertEquals(studentIDs.get("jackson"), studentIDs.get("julianna"));
    }

    @Test
    public void iteratorTest() {
        // replicate basic functionality test while building database
        HashMap<String, Integer> studentIDs = new HashMap<String, Integer>();
        studentIDs.put("christine", 12345);
        studentIDs.put("itai", 345);
        assertTrue(studentIDs.containsKey("christine"));
        assertTrue(studentIDs.containsKey("itai"));

        // ensure that containsKey does not always return true
        assertFalse(studentIDs.containsKey("joe"));
        assertFalse(studentIDs.containsKey("gigi"));
        assertFalse(studentIDs.containsKey("jon"));
        studentIDs.put("jon", 162);
        assertTrue(studentIDs.containsKey("jon"));

        // confirm hashMap can handle values being the same
        studentIDs.put("zoe", 12345);
        assertEquals(studentIDs.get("zoe"), studentIDs.get("christine"));

        HashSet<String> expected = new HashSet<String>();
        expected.add("christine");
        expected.add("itai");
        expected.add("jon");
        expected.add("zoe");

        HashSet<String> output = new HashSet<String>();
        for (String name : studentIDs) {
            output.add(name);
        }
        assertEquals(expected, output);
    }

}
